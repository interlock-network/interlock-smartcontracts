...
const toNode = (index: number | BigNumber, account: string, share: BigNumber, pool: BigNumber): Buffer => {
  const pairHex = utils.solidityKeccak256(['uint256', 'address', 'uint256', 'uint256'], [index, account, share, pool])
...
const verifyProof = (
  ...
  let pair = toNode(index, account, share, pool)
...
const getRoot = (balances: { account: string; share: BigNumber; pool: BigNumber; index: number }[]): Buffer => {
  let nodes = balances
    .map(({ account, share, pool, index }) => toNode(index, account, share, pool))
...
let balances: { index: number; account: string; share: BigNumber; pool: BigNumber }[] = []
let valid = true
...
Object.keys(json.claims).forEach((address) => {
  ...
  balances.push({ index: claim.index, account: address, share: BigNumber.from(claim.share), pool: BigNumber.from(claim.pool)})
  if (verifyProof(claim.index, address, claim.share, claim.pool, proof, merkleRoot)) {
