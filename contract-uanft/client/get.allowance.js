...
async function allowance(access_selector, owner, operator, id) {
	...
	try {
		...
		const { gasRequired, storageDeposit, result, output } =
			await contract.query['psp34::allowance'](
  			OWNER_pair.address, {}, owner, operator, {u16: id});
...
allowance(process.argv[2], process.argv[3], process.argv[4], process.argv[5]).then(() => process.exit());
